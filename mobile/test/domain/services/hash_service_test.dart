...
void main() {
  ...
  group('HashService _hashAssets', () {
    ...
    test('does not retry the same asset across multiple albums in one run', () async {
      final album1 = LocalAlbumStub.recent.copyWith(id: 'album-1');
      final album2 = LocalAlbumStub.recent.copyWith(id: 'album-2');
      final asset = LocalAssetStub.image1;
      ...
      when(
        () => mockNativeApi.hashAssets([asset.id], allowNetworkAccess: false),
      ).thenAnswer((_) async => [HashResult(assetId: asset.id, error: 'hash_failed')]);
