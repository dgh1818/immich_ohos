...
protocol NetworkApi {
  ...
  func setCaBundle(pemData: FlutterStandardTypedData, completion: @escaping (Result<Void, Error>) -> Void)
...
class NetworkApiSetup {
  ...
  static func setUp(binaryMessenger: FlutterBinaryMessenger, api: NetworkApi?, messageChannelSuffix: String = "") {
    ...
    let setCaBundleChannel = FlutterBasicMessageChannel(name: "dev.flutter.pigeon.immich_mobile.NetworkApi.setCaBundle\(channelSuffix)", binaryMessenger: binaryMessenger, codec: codec)
    if let api = api {
      setCaBundleChannel.setMessageHandler { message, reply in
        let args = message as! [Any?]
        let pemDataArg = args[0] as! FlutterStandardTypedData
        api.setCaBundle(pemData: pemDataArg) { result in
          switch result {
          case .success:
            reply(wrapResult(nil))
          case .failure(let error):
      ...
      setCaBundleChannel.setMessageHandler(nil)
    }
