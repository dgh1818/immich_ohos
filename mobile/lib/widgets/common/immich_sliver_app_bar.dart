        ...
        actions: [
          ...
          if (actions != null)
            ...actions!.map((action) => Padding(
                padding: const EdgeInsets.only(right: 16), child: action)),
          ...
          if (showUploadButton)
            const Padding(
                padding: EdgeInsets.only(right: 20), child: _BackupIndicator()),
          const Padding(
              padding: EdgeInsets.only(right: 20), child: _ProfileIndicator()),
        ],
      ),
    );
  }
      ...
      child: Badge(
        label: Container(
          ...
          child: const Icon(Icons.info,
              color: Color.fromARGB(255, 243, 188, 106), size: widgetSize / 2),
        ),
...
class _BackupIndicator extends ConsumerWidget {
  ...
  Widget? _getBackupBadgeIcon(BuildContext context, WidgetRef ref) {
    final backupStateStream =
        ref.watch(settingsProvider).watch(Setting.enableBackup);
  ...
  void initState() {
    ...
    _dismissalAnimation = Tween<double>(
      begin: 1.0,
        ...
        parent: _dismissalController, curve: Curves.easeOutQuart));
  }
