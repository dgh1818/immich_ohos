  ...
  Widget build(BuildContext context, WidgetRef ref) {
      ...
      if (oldControls?.pause != newControls.pause || newControls.restarted) {
        unawaited(() async {
          if (!newControls.pause) {
            await gCastService.preparePlaybackMetadata(
              asset,
              isCurrent: _isCurrentAsset(asset, ref.read(currentAssetNotifier)),
            );
          }
    ...
    void onPlaybackStatusChanged() {
      ...
      if (videoPlayback.state == VideoPlaybackState.playing) {
        unawaited(
          gCastService.preparePlaybackMetadata(
            asset,
            isCurrent: _isCurrentAsset(asset, ref.read(currentAssetNotifier)),
          ),
        );
    ...
    useEffect(() {
      gCastService.setCurrentPlaybackAsset(ref.read(currentAssetNotifier));
      return null;
    }, const []);

    ref.listen(currentAssetNotifier, (_, value) {
      gCastService.setCurrentPlaybackAsset(value);
      final playerController = controller.value;
      if (playerController != null && value != asset) {
        removeListeners(playerController);
        unawaited(gCastService.clearPreparedPlaybackSessionIfCurrent());
      }
