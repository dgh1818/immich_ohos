...
class ViewerTopAppBar extends ConsumerWidget implements PreferredSizeWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final showViewInTimelineButton =
        previousRouteName != TabShellRoute.name && previousRouteName != null;

    final isShowingSheet = ref
        .watch(assetViewerProvider.select((state) => state.showingBottomSheet));
    int opacity = ref
        .watch(assetViewerProvider.select((state) => state.backgroundOpacity));
    final showControls =
    ...
    final actions = <Widget>[
      ...
      if (asset.hasRemote && isOwner && asset.isFavorite)
        const UnFavoriteActionButton(
            source: ActionSource.viewer, menuItem: true),
