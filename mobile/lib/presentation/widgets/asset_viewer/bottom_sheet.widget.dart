...
class AssetDetailBottomSheet extends ConsumerWidget {
  ...
  const AssetDetailBottomSheet(
      {this.controller, this.initialChildSize = 0.35, super.key});
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    Future<void> editDateTime() async {
      await ref
          ...
          .editDateTime(ActionSource.viewer, context);
    }
        ...
        if (cameraTitle != null)
          _SheetTile(
            ...
            leading: Icon(Icons.camera_outlined,
                size: 24, color: context.textTheme.labelLarge?.color),
...
class _SheetAssetDescription extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<_SheetAssetDescription> createState() =>
      _SheetAssetDescriptionState();
}

class _SheetAssetDescriptionState
    extends ConsumerState<_SheetAssetDescription> {
  ...
  Future<void> saveDescription(String? previousDescription) async {
    ...
    final editAction = await ref
        .read(actionProvider.notifier)
        .updateDescription(ActionSource.viewer, newDescription);
