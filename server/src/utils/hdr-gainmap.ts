...
export type LegacyHdrGainmapType = 'cuva';
...
export function extractLegacyGainmap(data: Buffer): LegacyGainmapExtraction | null {
  const offsets = findLegacyGainmapOffsets(data);
  if (!offsets) {
    return null;
  }
...
function findLegacyGainmapOffsets(data: Buffer): LegacyGainmapOffsets | null {
